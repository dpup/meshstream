...
// FormatMapReportMessage formats a MapReport message
func FormatMapReportMessage(mapReport *pb.MapReport) string {
	var builder strings.Builder

	if mapReport == nil {
		return "Error: nil map report data"
	}

	builder.WriteString("Map Report Data:\n")

	// Node information
	if mapReport.GetLongName() != "" {
		builder.WriteString(fmt.Sprintf("  Long Name: %s\n", mapReport.GetLongName()))
	}

	if mapReport.GetShortName() != "" {
		builder.WriteString(fmt.Sprintf("  Short Name: %s\n", mapReport.GetShortName()))
	}

	if mapReport.GetRole() != 0 {
		builder.WriteString(fmt.Sprintf("  Role: %s\n", mapReport.GetRole().String()))
	}

	if mapReport.GetHwModel() != pb.HardwareModel_UNSET {
		builder.WriteString(fmt.Sprintf("  Hardware: %s\n", mapReport.GetHwModel().String()))
	}

	if mapReport.GetFirmwareVersion() != "" {
		builder.WriteString(fmt.Sprintf("  Firmware: %s\n", mapReport.GetFirmwareVersion()))
	}

	if mapReport.GetRegion() != 0 {
		builder.WriteString(fmt.Sprintf("  Region: %s\n", mapReport.GetRegion().String()))
	}

	if mapReport.GetModemPreset() != 0 {
		builder.WriteString(fmt.Sprintf("  Modem Preset: %s\n", mapReport.GetModemPreset().String()))
	}

	builder.WriteString(fmt.Sprintf("  Default Channel: %v\n", mapReport.GetHasDefaultChannel()))

	// Position information
	if mapReport.GetLatitudeI() != 0 && mapReport.GetLongitudeI() != 0 {
		// Convert the integer coordinates to floating-point degrees
		// Meshtastic uses a format where the values are stored as integers
		// representing the position multiplied by 1e7 (10 million)
		lat := float64(mapReport.GetLatitudeI()) / 10000000.0
		lon := float64(mapReport.GetLongitudeI()) / 10000000.0
