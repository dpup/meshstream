...
func FormatTopicAndMapData(topicInfo *TopicInfo, decodedPacket *DecodedPacket) string {
	var builder strings.Builder
	...
	if decodedPacket.DecodeError != nil {
		builder.WriteString(fmt.Sprintf("Error decoding packet: %v\n", decodedPacket.DecodeError))
		if decodedPacket.RawEnvelope != nil {
			// Try to format whatever we got
			builder.WriteString(FormatServiceEnvelope(decodedPacket.RawEnvelope))
		}
		return builder.String()
	}
	...
	// First show basic packet info
	builder.WriteString(fmt.Sprintf("  Channel ID: %s\n", decodedPacket.ChannelID))
	if decodedPacket.GatewayID != "" {
		builder.WriteString(fmt.Sprintf("  Gateway ID: %s\n", decodedPacket.GatewayID))
	}
	
	// Node information
	builder.WriteString(fmt.Sprintf("  From Node: %d\n", decodedPacket.From))
	if decodedPacket.To != 0 && decodedPacket.To != 4294967295 { // 4294967295 is broadcast
		builder.WriteString(fmt.Sprintf("  To Node: %d\n", decodedPacket.To))
	} else {
		builder.WriteString("  To: Broadcast\n")
	}
	
	// Check if this is a MAP_REPORT_APP port and format payload accordingly
	if decodedPacket.PortNum == pb.PortNum_MAP_REPORT_APP {
		// Format the payload based on its type
		builder.WriteString(FormatPayload(decodedPacket.Payload, decodedPacket.PortNum))
	} else {
		builder.WriteString(fmt.Sprintf("\n  Unexpected port number %s for map data\n", decodedPacket.PortNum))
	}
